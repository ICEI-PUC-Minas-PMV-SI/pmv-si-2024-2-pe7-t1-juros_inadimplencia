...
dados_inflac = dados.drop(['Confianca_Valor', 'Endividamento_Total', 'Selic_Valor'],axis=1)
...
plt.xlabel('Periodo')
plt.ylabel('Inflacao_Acumulada')
plt.tight_layout()

# Definição do treinamento e testes do modelo
dados_treino = dados_inflac[dados_inflac.index < '2019-06-01']
dados_teste = dados_inflac[dados_inflac.index >= '2019-06-01']

# Verificar estacionariedade (resultado precisa ser menor que 0.05)
result = adfuller(dados_inflac)
...
plt.title("Série Temporal - Inflação Acumulada últimos 12 meses (Treinamento e Teste)")
plt.legend()
plt.xlabel("Período")
...
plt.figure(figsize=(12, 6))
plt.plot(dados_selic['Inflacao_Acumulada'], label="Histórico", color="blue")
...
plt.title("Previsão da Inflação acumulada últimos 12 meses - Próximos 36 Meses")
plt.legend()
plt.xlabel("Período")
plt.ylabel("Inflação")
plt.grid()
